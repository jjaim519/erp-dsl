...
radius              : xs sm md full             (6/12/20/알약 — 단 이름 = 크기 대역)
...
| **줄의 상태는 값이 말한다** | 반복 항목에 `@mark: 'del'` 같은 키를 실으면 그 줄의 칸 전부에 `mk-del` 클래스가 붙는다. 서식은 안 건드린다 — **어떤 줄이 지워졌나는 값이 와야 안다.** DSL 은 이름만 옮기고 색·취소선은 소비처 CSS 가 정한다(`.erpPaperDoc .erpPaperCell.mk-del { ... }`). 낱말은 `^[a-z][a-z0-9-]*$` 만 통과한다 |
| **칸 하나만 가리킬 때** | `@mark.<필드 끝마디>` (예: `@mark.배송요청일: 'chg'`). 걸침 칸·그룹머리에도 실린다 — 줄 표시(`@mark`)는 「이 줄이 바뀌었다」이고 이쪽은 「이 칸이 바뀌었다」다 |
...
`--rows`는 **쪽당 행 수**이고 이 하나가 행 높이와 글자 크기를 함께 정한다(42→9pt / 31→12pt).
...
쥔 쪽만 안다. 반복 안은 점 경로로 적는다(`'품목.품명'` = 그 필드의 전 행).

한 배열에서 **일부 행만** 잠글 땐 행 번호를 붙인다 — `'품목[14].품명'`(원본 배열 순번).
둘은 OR라 전체 잠금이 있으면 인덱스로 «풀» 수 없다. 잠긴 줄과 직접 쓰는 빈 줄이 한 표에
섞이는 경우(양식이 깐 고정 줄 + 그 아래 자유 입력 줄)가 이걸 쓰는 자리다.

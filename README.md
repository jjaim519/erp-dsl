...
트리에서는 「그룹머리」 없이 **「그룹꼬리」만 적어도 소계가 된다** — 깊이 1인 줄이 다시 나오는 것이
곧 앞 묶음의 끝이라 묶음 기준을 구조가 이미 말하고 있다. ⚠ 소계는 그 묶음에 딸린 **모든 줄**을
더한다(중간 줄도 자기 금액을 갖는 내역서 전제) — 부모를 자식의 합으로 넣으면 두 번 더해진다.
**깊이별 소계(레벨 2에도 소계)는 아직 안 연다** — 반복 하나에 그룹꼬리 하나가 지금 엔진의 전제다.
실물: `public/paper-sample-tree-ledger.xlsx`.

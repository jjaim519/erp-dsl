...
//   · A층 — 데이터·콜백 유무
//     행 전체 클릭=`onSelect`(DataTable·ListWidget과 같은 규율) · 선택 표시=`selectedId` · 담당 열=item.owner · 만기·연령
//     열=item.due/age · 로딩=`status`
//   · 행에 경로는 하나
//     행 액션 열이 없다. 처음엔 chevron·첫 셀 링크·「수금」 버튼을 같이 뒀는데, 첫 셀에 링크를 건 순간 액션 열이 갈 곳을 잃었다(같은 행에서 두 경로가 같은 곳을 가리킨다) —
//     실제로 `stopPropagation` 해킹도 필요했다. `DataTable`이 이미 주석으로 못박아 둔 규칙이다: "행 클릭 이동(범용). actions 셀과 경쟁 안 하도록 보기
//     액션은 두지 않음." 행 단위 액션이 꼭 필요한 화면이면 행 클릭을 포기하고 `DataTable`을 쓴다
//   · 헤더를 안 갖는다
//     제목·설명·헤더 액션 prop이 없다 — 그건 `PageHeader`가 소유한 자리이고, 부품이 페이지 헤더를 한 벌 더 들면 같은 자리를 둘이 다툰다. 첫 판에 소비처 화면의 헤더
//     행(제목·엑셀·총계)을 그대로 옮겼다가 회수했다. 총계는 하단 합계행(AgingReportWidget tfoot과 같은 규율)
//   · 상세를 담는 표면
//     상세는 `Modal size='full'`(95vw). 좁은 Drawer(잔액 열이 화면 밖)와 5:7 2-pane(좌측 대상명·우측 적요 잘림)을 둘 다 만들어 보고 되돌렸고, 별도
//     페이지도 검토했으나 상세가 표 하나뿐이라 라우트를 파는 값이 안 나온다는 오너 판단으로 모달로 굳혔다. md/lg 폭에서는 7열 원장이 잘리므로 열 수로 고른다: 7열 이상(통장내역)은
//     `full`, 5열 안팎(수납 원장)은 `lg` — full에 5열을 넣으면 적요 칸이 텅 빈다. (전엔 잔액 열을 sticky로 붙여 뒀는데, 그건 폐기한 2-pane 실험용이었고 열이
//     늘자 ✓ 열을 덮어서 걷어냈다)

...
export const CATALOG: CatalogEntry[] = [
  ...
  { name: 'OptionSetPicker', layer: '유기체', role: '저작 면이 정의한 OptionSet을 읽어 *고르는* 선택 면 위젯 — OptionSetEditor의 짝. **표현 어휘(닫힌 집합, ListWidget align 문법 — 자동 도출+override)**: single→chips(≤5 자동)/grid(6~10, 2열)/list/segmented/select · quantity→stepper/grid(>8 자동) · number→input(범위 넓거나 무한 자동 — 타이핑, blur에 min/max 스냅)/stepper. 표현은 데이터가 아님(§1 불변 — CalendarPage encoding 동형). 3모드(idle/pick/configure)는 소비처가 정함. CTA 잠금 절충: 필수 미충족 시 잠금 스타일이되 탭은 받아 첫 미충족 그룹 스크롤+강조. 그룹 헤더 배지 전환("필수"→체크+선택 요약). 풋터 고정·스크롤 내부(§6-4). 금액 계산 0(§6).',
    props: [
      { name: 'mode', kind: '기능', values: "'idle'(placeholder) | 'pick'(title·items·onPick·emptyState?) | 'configure'(아래 전부)" },
      { name: 'groups / selection', kind: '기능', values: 'OptionGroup[] / OptionSelection { picked, qty, nums } (controlled)' },
      { name: 'display?', kind: '스타일', values: "Record<groupId, 'list'|'chips'|'grid'|'segmented'|'select'|'stepper'|'input'> — 그룹별 표현 override(생략=자동 도출, selection과 안 맞으면 무시)" },
      { name: 'onPick / onQty / onNum', kind: '기능', values: '(groupId, code) / (choiceId, qty) / (fieldKey, value) — 선택 3축과 1:1' },
      { name: 'title / meta? / quantity?', kind: '콘텐츠', values: 'string / string / { value, onChange } — 헤더(대상 제목+전체 수량)' },
    ...
    composition: {
      토큰: ['surface-raised+elevation', 'primary-0(선택 틴트)·success(충족 체크)', 'flash 키프레임(reduced-motion 대응)', 'scrollbar-gutter', '밀도 행(32px min, 실화면 검증)'],
      '의미 원자': ['Title', 'Text', 'Badge', 'Icon', 'Button', 'Chip', 'SegmentedControl', 'Select', 'NumberInput'],
      분자: ['NumberStepper'],
      유기체: ['EmptyState'],
      공유: ['optionset(공유 타입)', '_cells(fmtCurrency)'],
    } },
  { name: 'CompositionOutline', layer: '유기체', role: '2-pane 작성 면의 우측 "작성물 카드 스택" — 카드는 라인이 있거나 작성 중(active)인 섹션만(빈 섹션 상시 노출은 폐기 — 소비처 실화면에서 섹션 다수면 우측이 후보 메뉴로 도배됨, 2026-07-27 반전). 추가 진입점은 상단 단일 버튼 + 섹션 선택 Popover 메뉴(부품 소유) → onAddToSection 그대로 발화. 읽는 물건이 아니라 조작면 — 라인 클릭=좌측 재진입(onSelectLine)·active=tint+inset 링(좌측 강조선 금지). 합계 블록은 하단 고정, value는 이미 포맷된 문자열(§6-1 — 부품은 계산 0). footer=저장 영역 완전 위임. 멘탈모델: Stripe 인보이스 편집기.',
    props: [
      { name: 'sections', kind: '기능', values: 'CompositionSection[] = { id, label, badge?, lines, active? } — 전체가 메뉴 후보, 카드는 라인 보유·active만. addLabel?은 지원 중단(무시)' },
      ...
      { name: 'onAddToSection / onSelectLine / onDeleteLine', kind: '기능', values: '(sectionId) / (lineId) / (lineId) — 삭제는 행 hover 노출' },
    ],
    composition: {

...
export const CATALOG: CatalogEntry[] = [
  ...
  { name: 'MobilePaperViewer', layer: '유기체', role: '폰의 **A4 장표 뷰어** — 데스크탑 PaperModal의 *형제*(축소판 아님). A4 CANON 794px를 폰 폭 393pt에 맞추면 배율 0.495인데 캔버스는 인쇄 좌표계라 데스크탑 타이포(14px)를 써서 글자가 6.9px가 된다 → fit-to-width 하나로는 못 읽는 화면이라 **두 뷰**를 갖는다: ① 읽기(기본) = rows를 라벨-값으로 **투영**(AI 추론이 아니다 — 구조를 이미 갖고 있다. rowSpan 라벨은 그룹 머리로 내려가고 colSpan은 손실 0, image/node만 자리가 사라진다) / ② 원본 = CANON 캔버스 2D 스크롤 + **핀치 확대**(WCAG 1.4.10이 2차원 배치 콘텐츠를 예외로 두는 그 자리 — 예외는 본체에만 걸리고 크롬은 rem 토큰). 팬은 브라우저 기본 스크롤이고 핀치만 직접 잡는다(touch-action: pan-x pan-y). 핀치는 multipoint라 **WCAG 2.5.1(Level A)** 이 단일 포인터 대안을 요구하므로 **하단 확대율 표기 자체가 버튼**이다(누르면 맞춤 → 실제 → 2배 순환). 배율이 연속인 것은 헌법 5와 안 부딪힌다 — 그 규율은 API 표면에 대한 것이고 `zoom` prop을 안 연다. 업계 수렴: Word·Docs·DocuSign·Acrobat이 전부 두 뷰를 갖고, DocuSign은 리플로우가 \'재무 양식처럼 표현의 정밀함이 중요한 문서엔 늘 적합하진 않다\'고 스스로 단서를 단다. 전체 화면 커버(모달 아님) — 크롬 규범은 MobileAttachmentViewer와 한 벌.',
    props: [

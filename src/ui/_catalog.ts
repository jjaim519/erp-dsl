...
export const CATALOG: CatalogEntry[] = [
  ...
  { name: 'PaperDocModal', layer: '유기체', role: '문서를 **보고·인쇄하고·채우는** 모달. PaperModal의 형제이지 재사용이 아니다 — 그쪽 인쇄 빌트인은 `.erpPaper` 하나를 position:fixed로 물리 A4에 고정해서 **여러 장이 0,0에 겹쳐 한 장으로 나간다**. 그래서 인쇄는 PaperDoc의 경로(장마다 break-after)를 쓰고 이 모달은 «문서 밖을 감추는 일»만 더한다 — 그 규칙은 반드시 `html.erp-doc-print` 안에 갇혀 있다(스코프 없는 `body * {visibility:hidden}`이 소비 앱 전 화면을 백지로 만든 적이 있다). 보기/편집은 한 부품(mode) — 문서 기하가 같기 때문. **편집 초안은 모달이 쥔다**: 소비처는 onSave 하나만 배선하고, 저장 안 한 채 닫으려 하면 푸터가 확인으로 바뀐다(모달 위에 모달을 쌓지 않는다).',
    props: [
      { name: 'opened / onClose', kind: '기능', values: 'boolean, () => void' },
      { name: 'title', kind: '콘텐츠', values: 'string' },
      { name: 'spec / values', kind: '콘텐츠', values: 'PaperSpec, Record<string, unknown> (값↔DB 매핑은 소비처 몫 — 서식은 «어떤 이름이 필요한가»만 말한다)' },
    ...
    composition: {
      토큰: ['모달 폭 = **종이 폭**(세로 A4면 818px, 좁은 창에서만 95vw 상한)', '데스크 패딩 12px(명시예외)', '--bg-secondary(데스크)', '배율 맞춤(한 장 통째)/100% — **보기에만** 있다. 편집엔 100%만(축소해 놓고 글자를 칠 순 없다) · 인쇄도 편집엔 없다', '푸터: 닫기·취소 최좌측 / 우측 인쇄 → 소비처 actions → 저장', '@media print: html.erp-doc-print 스코프 안에서만 문서 밖 감추기'],
      '의미 원자': ['Title', 'Text', 'Icon(x)', 'Button', 'SegmentedControl(배율)'],
  ...
  { name: 'MobilePaperViewer', layer: '유기체', role: '폰의 **A4 문서 뷰어** — 데스크탑 PaperDocModal·PaperModal의 *형제*(축소판 아님). 하는 일은 하나다: **인쇄 좌표계로 그려진 문서를 폰에서 훑어보게 한다.** 그리지도 다시 쓰지도 않는다. ⚠ v0.75의 「읽기 뷰」(장표를 라벨-값으로 투영)는 **v0.77.0에서 걷어냈다** — 문서 체계가 PaperSpec으로 옮겨오면서 근거가 사라졌기 때문이다: `PaperCell`은 좌표와 text/field/border만 갖고 라벨-값 짝은 "왼쪽 칸이 라벨"이라는 **시각적 인접성 추론**으로만 성립한다(= 되살리면 Adobe Liquid Mode와 같은 처지가 된다). 실제로 소비처는 그걸 피하려고 폰 전용 2열 격자를 손으로 합성해 넘겼고, 그래서 **「원본」 탭이 원본이 아니었다.** 투영을 안 하면 구조가 필요 없으므로 **계약은 `children`으로 돌아왔다**(PaperModal과 한 벌) — `<PaperDoc spec values />`도, 소비처가 이미 가진 A4 문서도 그대로 들어온다(폰용으로 문서를 다시 짜는 일이 없어진다 — 그게 두 화면을 갈리게 하던 원인이다). 확대: **폰에서는 폭이 늘 구속조건**이라 폭맞춤이 곧 「한 장 전체」다(세로 A4 한 장 556px < 무대 659~739px) → 열자마자 폭맞춤이고, **더블탭으로 폭맞춤 ↔ 100%**(탭 지점 앵커), 핀치는 그 사이를 연속으로 지난다. 핀치는 multipoint라 **WCAG 2.5.1(Level A)** 이 단일 포인터 대안을 요구하므로 **하단 확대율 표기 자체가 버튼**이다(맞춤 → 100% → 200% 순환). WCAG **1.4.10 Reflow**가 2차원 배치 콘텐츠(데이터 표를 명시적 예로 든다)를 예외로 두는 그 자리이고, 예외는 본체에만 걸리므로 크롬은 전부 rem 토큰이다. 팬은 브라우저 기본 스크롤이고 핀치만 직접 잡는다(touch-action: pan-x pan-y). 배율이 연속인 것은 헌법 5와 안 부딪힌다 — 그 규율은 API 표면이고 `zoom` prop을 안 연다. 전체 화면 커버(모달 아님) — 크롬 규범은 MobileAttachmentViewer와 한 벌. **인쇄 스코프는 없다**: 「문서 밖 치우기」는 화면을 소유한 쪽만 할 수 있는 일이라(PaperDocModal은 Portal이라 가능하지만 이 커버는 앱 트리 안이라 같은 규칙이 자기를 지운다) 인쇄는 소비처 몫이다(paper.css doctrine).',
    props: [

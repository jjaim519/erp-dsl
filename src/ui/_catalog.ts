...
export const CATALOG: CatalogEntry[] = [
  ...
  { name: 'PaperDoc', layer: '유기체', role: '서식(PaperSpec) + 값 → A4 **여러 장**. 도메인 0줄. 배치(쪽 나눔·반복 펼침·집계·묶음 걸침)는 순수 엔진(paperLayout)이 하고 여기는 그리기만 한다. 24열 격자는 *정렬 골격*이고 **선은 셀의 속성**이다 — 선 소유권(각 칸이 자기 위·왼쪽만 그린다)으로 맞닿은 자리가 2px 되는 걸 막는다. 괘선은 종이 절대색(검정) — 역할 변수를 태우면 음영 칸 위에서 선이 사라진다. `mode: edit`이면 데이터 자리가 입력이 되되 **문서 기하는 그대로**(칸이 곧 입력의 크기, 입력은 자기 크롬을 벗는다). 서식은 `npx erp-paper-import`로 xlsx에서 만든다. ⚠ 2단 그룹 미해결(걸침·그룹머리가 두 층이면 안쪽이 바깥을 따라가 *틀린 값*이 찍힌다) · 흐름 셀 없음.',
    props: [

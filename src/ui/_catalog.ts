...
export const CATALOG: CatalogEntry[] = [
  ...
  { name: 'OptionSetPicker', layer: '유기체', role: '저작 면이 정의한 OptionSet을 읽어 *고르는* 선택 면 위젯 — OptionSetEditor의 짝. selection별 대응: single=단일선택 행 / quantity=스테퍼 행 / number=수치 스테퍼(min/max/step 이탈 불가 입력기, step 미지정=1). 3모드(idle/pick/configure)는 소비처가 정함(부품은 단계 판단 없음). CTA 잠금 절충: 필수 미충족 시 잠금 스타일이되 탭은 받아 첫 미충족 그룹 스크롤+강조. 그룹 헤더 배지 전환("필수"→체크+선택 요약). 풋터(소계+CTA) 고정·스크롤은 내부(§6-4). 금액 계산 0(§6).',
    props: [
    ...
    composition: {
      토큰: ['surface-raised+elevation', 'primary-0(선택 틴트)·success(충족 체크)', 'flash 키프레임(reduced-motion 대응)', 'scrollbar-gutter'],
      '의미 원자': ['Title', 'Text', 'Badge', 'Icon', 'Button'],
      분자: ['NumberStepper'],
      유기체: ['EmptyState'],
      공유: ['optionset(공유 타입)', '_cells(fmtCurrency)'],
    } },
  { name: 'CompositionOutline', layer: '유기체', role: '2-pane 작성 면의 우측 "문서 목차" 위젯 — 카트가 아니라 목차: 라인 0개여도 모든 섹션이 헤더+추가와 함께 보인다(구조가 먼저, 내용이 나중). 읽는 물건이 아니라 조작면 — 라인 클릭=좌측 재진입(onSelectLine)·섹션 추가=좌측 진입점(onAddToSection)·active=tint+inset 링(좌측 강조선 금지). 합계 블록은 하단 고정, value는 이미 포맷된 문자열(§6-1 — 부품은 계산 0). footer=저장 영역 완전 위임. 멘탈모델: Stripe 인보이스 편집기.',
    props: [
      { name: 'sections', kind: '기능', values: 'CompositionSection[] = { id, label, badge?, addLabel?, lines, active? } — badge는 불투명 문자열' },

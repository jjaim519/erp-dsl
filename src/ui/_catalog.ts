...
export const CATALOG: CatalogEntry[] = [
  ...
  { name: 'OptionSetEditor', layer: '유기체', role: '선택지 묶음(OptionSet)을 *정의*하는 저작 면 위젯 — OptionSetPicker의 짝(같은 타입을 쓰고 읽는다, 변환 계층 금지). Repeater raw 슬롯을 닫아 "같은 편집기가 화면마다 다르게 생기는 것"을 차단. 그룹=Repeater(접이: selection 배지+값 개수+×배율 요약) 안 값=Repeater(평면: 정체 4칸+InheritedValueField+수량식/보정 옵션 행). 도메인 무지(헌법 1): section=표시 문자열·selection 4종(single/multi/quantity/number)만 안다, 금액 계산 0(§6). 필수는 number 외 전 선택형에 유의미(0건=미충족). 멘탈모델: Square Modifier Groups / Shopify 옵션 편집.',
    props: [
      { name: 'groups / onChange', kind: '기능', values: 'OptionGroup[] (controlled) / (groups)=>void — 모든 쓰기는 이 하나' },
      { name: 'sections', kind: '기능', values: '{ key, label, note?, description? }[] — 표시할 섹션과 순서. 없는 section의 그룹은 미렌더, 빈 배열=아무것도 안 그림' },
      { name: 'refOptions?', kind: '기능', values: 'RefOption[] — 값의 참조 SSOT 후보(InheritedValueField로 전달). 없으면 값/배율 직접 입력' },
      { name: 'exprVariables?', kind: '기능', values: 'ExprVariable[] — 있으면 값 행에 수량식(ExpressionField) 노출' },
      { name: 'adjustKeys?', kind: '기능', values: 'KVKey[] — 있으면 값 행에 보정(KeyValueField) 노출' },
      { name: 'sectionActions?', kind: '콘텐츠', values: '(sectionKey)=>ReactNode — 섹션 헤더 우측 완전 위임(가져오기 등)' },
      { name: 'readOnly? / emptyState?', kind: '기능', values: 'boolean / { title, description? }' },
    ],
    composition: {
  ...
  { name: 'OptionSetPicker', layer: '유기체', role: '저작 면이 정의한 OptionSet을 읽어 *고르는* 선택 면 위젯 — OptionSetEditor의 짝. **표현 어휘(자동 도출+override)**: single·multi→chips(≤5)/grid(6~10)/filtered(>10·값묶음 有 — 필터 칩+단일 목록)/list/segmented/select · quantity→stepper/collect(>10 자동 — 빈 행=행 전체 버튼·클릭=1, 담긴 행=✓·스테퍼·행 소계(amount×수량, §6 표시 산술 예외)·단위·상단 고정)/grid · number→input(범위 넓거나 무한 — 타이핑, blur 스냅)/stepper. **접힘 모델**: 선택은 접힘을 유발하지 않음(횡단 비교 보존) — 초기값 defaultCollapsed(기본 satisfied)+"다음" 진행 접힘+수동 토글, collapseOnPick은 opt-in, openGroups로 controlled 승격. 그룹 헤더 sticky·필수=라벨 별표·그룹 1개+섹션 없음=헤더 자동 생략(골라 담는 면). 검색: 소비처 제어(끄기·임계·문구), 대상=라벨+보조+값묶음(정규화), 미충족 필수 그룹은 검색이 못 지움. 스크롤·강조는 패널 내부만. CTA 잠금 절충(탭→첫 미충족 스크롤). 금액 계산 0(§6).',
    props: [
      ...
      { name: 'display?', kind: '스타일', values: "Record<groupId, 'list'|'chips'|'grid'|'segmented'|'select'|'filtered'|'stepper'|'collect'|'input'> — override(생략=자동, selection과 안 맞으면 무시)" },
      { name: 'onPick / onPickMany? / onQty / onNum', kind: '기능', values: '(groupId, code) / (groupId, codes[]) / (choiceId, qty) / (fieldKey, value) — 선택 4축과 1:1' },
      { name: 'title / path? / meta? / quantity?', kind: '콘텐츠', values: 'string / string[](경로 제목 — 길면 앞 생략 "… › a › b", 전체는 툴팁) / string / { value, onChange }(생략=헤더 스테퍼 미노출)' },
      { name: 'search?', kind: '기능', values: 'false | { threshold?(기본 12), placeholder? } — 소비처가 끄거나 조정' },
      { name: 'defaultCollapsed? / collapseOnPick?', kind: '기능', values: "'satisfied'(기본 — 필수 미충족·담긴 그룹만 펼침) | 'all' | 'none' | string[] / boolean(기본 false)" },
      { name: 'openGroups? / onToggleGroup?', kind: '기능', values: 'string[] / (groupId, open) — controlled 승격(둘 다 줄 때)' },
      { name: 'onUnmetChange?', kind: '기능', values: '(groupIds[]) — 미충족 목록 스트림(소비처 진행 표시용)' },
      { name: 'ref', kind: '기능', values: 'OptionSetPickerHandle { focusGroup(id), focusUnmet() } — 카드 밖에서 이동 명령' },
      { name: 'subtotal', kind: '값', values: 'number — 표시만(§6, 계산은 소비처)' },
      { name: 'primary / secondary? / blockedHint?', kind: '기능', values: '{ label, onClick } ×2 / string — 필수 미충족 사유(상시 노출)' },
    ],
    composition: {
      토큰: ['surface-raised+elevation', 'primary-0(선택 틴트)·success(충족 체크)', 'flash 키프레임(reduced-motion 대응)', 'scrollbar-gutter', 'sticky 그룹 헤더', '밀도 행(32px)'],
      '의미 원자': ['Title', 'Text', 'Icon', 'Button', 'Chip', 'SegmentedControl', 'Select', 'NumberInput'],
      분자: ['NumberStepper'],
      유기체: ['EmptyState'],
      공유: ['optionset(공유 타입)', '_cells(fmtCurrency)'],
    } },
  { name: 'CompositionOutline', layer: '유기체', role: '2-pane 작성 면의 우측 "작성물 카드 스택" — 카드는 라인이 있거나 작성 중(active)인 섹션만(빈 섹션 상시 노출 폐기 — 실화면 반전). 추가 진입점은 상단 단일 버튼 + **계층 메뉴**(부품 소유 Popover): items 있는 섹션은 하위까지 드릴(뒤로 포함)→onAddToSection(sectionId, itemId), 없는 섹션은 (sectionId)만 — 좌측 pick 모드 없이 곧장 구성/담기 면 진입. 조작면 — 라인 클릭=좌측 재진입, active=은은한 채움 한 겹+"편집 중" 라벨(카드 링 안에 링 중첩 금지·카운트 뱃지 없음). 선 최소화(라인 구분선 없음), 섹션 라벨=캡션 오버라인(라인이 주인공). 합계 하단 고정, value는 포맷된 문자열(§6-1). footer=완전 위임. 카드 flex:none(압착 방지).',
    props: [

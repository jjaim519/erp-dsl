...
export const CATALOG: CatalogEntry[] = [
  ...
  { name: 'Register', layer: '유기체', role: '**원장** — 이월 → 부호 있는 증감 → 행마다 누계 → 기말. 재무 원장(통장내역·계정별원장)과 재고 수불부가 **같은 골격**이라 `labels`·`unit`만 갈린다. 실물 기준(QuickBooks bank register · Dynamics BC Bank Account Ledger Entries 화면 확인): 기말잔액은 표 합계행이 아니라 **헤더 우상단**, 새 줄 입력은 **표 맨 위**(누계가 아래로 흐르니 밑에 있으면 새 줄이 누계의 *끝*처럼 읽힌다), 열 순서는 출금 → 입금 → ✓대사 → 잔액. **DataSheet와 형제인 선: 전기된 기록은 못 고친다**(정정은 반대 전표) — 그래서 행 편집 기제가 통째로 없고, 대신 DataSheet에 없는 세 축(누계·기간경계·대사)이 있다. 누계는 `balance`를 안 주면 **이월 + Σ(입−출)**로 부품이 만든다(그게 존재 이유다).',
    props: [
      ...
      { name: 'A층 — 데이터·콜백 유무', kind: '기능', values: '입력행=`onAdd` · 이월행=`carryOver` · 기말잔액=`closing`(value 없으면 누계에서 파생) · ✓열=entry.reconciled가 한 줄이라도 있으면 · ✓클릭=`onReconcile` · 대사뱃지=`reconciledThrough` · 기간네비=`period`+`onPeriodChange` · 계좌셀렉트=`accounts` 2개 이상(1개면 텍스트) · 두 줄 행=ref/sublabel/kindSub · 구분알약=entry.kind' },
      ...
      { name: 'B층 — actions', kind: '기능', values: 'Action[] (헤더 우측). 길이가 0이면 자리 자체가 없다' },
  ...
  { name: 'OpenItemList', layer: '유기체', role: '**미결 항목 목록**(SAP open item management) — 한 줄 = 아직 안 끝난 돈 한 건: 원금액 / 수납 / 잔액. 매출채권(수금)과 매입채무(지급)가 부호만 반대인 같은 물건이라 `labels`로 갈린다. **AgingReport와 갈리는 선**: 거기는 매트릭스라 회계가 *읽고*, 여기는 평면 목록 + 행동이라 담당자가 *처리한다*(DataTable/DataSheet 선례). **표면을 안 갖는다** — 행을 열면 무엇이 뜨는지는 페이지가 정하고(`onSelect`로 신호만), 한 건의 수납 이력은 `Register`가 **다른 화면에서 전체 폭으로** 그린다 — 좁은 Drawer도 2-pane도 안 된다(둘 다 만들어 보고 되돌렸다: 1200px을 5:7로 갈라도 좌측 대상명과 우측 적요가 잘린다). 업계도 분할이 아니라 이동이다(SAP List Report→Object Page · Odoo/Xero 목록→상세). 소비처 화면 진단에서 나온 부품이다: 현장 목록(계약금액/수금액/미수금) → 한 건 열기 → 수납 기록+첨부 구조가 도메인 특수가 아니라 업계 보편이었다.',
    props: [
      ...
      { name: 'A층 — 데이터·콜백 유무', kind: '기능', values: '행 클릭=`onSelect` · 선택 표시=`selectedId`(2-pane) · 담당 열=item.owner · 만기·연령 열=item.due/age · 로딩=`status`' },
      { name: 'B층 — itemActions', kind: '기능', values: '`itemActions(item) => Action[]` — 안 주면 열 자체가 없다. 행에 경로가 둘인데 **하는 일이 다르다**: chevron(=`onSelect`)은 그 건의 원장으로 *이동*, 버튼은 그 자리에서 짧은 일(수납 한 건 기록 = Modal + 폼. 네 칸짜리 폼에 새 부품을 파지 않는다). 같은 목적지로 가는 두 경로였다면 경쟁이라 금지다(§11-3)' },
      ...
      { name: '상세를 담는 표면', kind: '값', values: '**목록과 원장을 한 화면에 놓지 않는다.** 좁은 Drawer도 2-pane도 안 된다 — 둘 다 만들어 화면으로 확인하고 되돌렸다(서랍은 잔액 열이 화면 밖, 5:7 분할은 좌측 대상명과 우측 적요가 잘림). 업계도 분할이 아니라 **이동**이다(SAP List Report→Object Page · Odoo/Xero 목록→상세 폼). Flexible Column Layout은 상세가 *폼*일 때 쓰는 것이지 7열 원장 자리가 아니다. 그래도 Register의 잔액 열은 sticky로 고정했다 — 어떤 폭에서도 결론은 안 사라져야 한다' },
  ...
  { name: 'AgingReport', layer: '유기체', role: '**채권 연령 매트릭스** — 행=거래처(펼치면 청구건) × 열=연령 버킷 + 합계. 업계 형태 그대로(Odoo Aged Receivable · Xero AR Aging · SAP "AR Aging Grid"): **버킷은 필터가 아니라 열**이다. 한 거래처의 돈이 어느 연령대에 깔려 있는지는 가로로, 어느 연령대가 회사 전체에서 두꺼운지는 세로로 읽는데, 버킷을 상단 필터 밴드로 접으면 그 두 읽기가 모두 사라진다(첫 목업에서 그렇게 만들었다가 되돌렸다). **행동은 안 갖는다** — 회계가 *읽는* 리포트고, 영업이 *처리하는* 수금 큐는 별개 부품(같은 데이터를 두 부품이 다르게 보는 건 정상 — DataTable/DataSheet 선례). 색은 값이 아니라 **구간**에 붙는다(값에 색을 걸면 같은 금액이 열마다 다른 색이 되어 눈이 금액 크기로 읽어 버린다).',
    props: [
      ...
      { name: 'B층 — actions', kind: '기능', values: 'Action[] (헤더)' },

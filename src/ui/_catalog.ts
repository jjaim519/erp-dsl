...
export const CATALOG: CatalogEntry[] = [
  ...
  { name: 'DocModal', layer: '유기체', role: '회사 서류의 껍데기 하나.',
    props: [
      { name: 'opened / onClose', kind: '기능', values: 'boolean, () => void' },
      { name: 'title', kind: '콘텐츠', values: 'string' },
      { name: 'spec / values', kind: '콘텐츠', values: 'PaperSpec, Record<string, unknown> — children과 택일. 값↔DB 매핑은 소비처 몫이다(서식은 어떤 이름이 필요한가만 말한다)' },
      ...
      { name: 'toolbar', kind: '콘텐츠', values: 'ReactNode — 헤더 우측(제목 반대편) 열린 슬롯. 문서의 종류를 고르는 자리다(견적서 갑/을). `views: {label,value}[]` 같은 모양을 못박지 않은 이유: 어떤 도메인은 토글이고 어떤 도메인은 라디오·선택이다 — 못박으면 그 도메인들이 각자 껍데기를 다시 만든다. ⚠ 행위(인쇄·닫기)는 넣지 않는다(빌트인이고, 섞으면 헤더가 두 문법이 된다)' },
  ...
  { name: 'MobileListRow', layer: '분자', role: '모바일 목록의 행.',
    props: [
      ...
      { name: 'emphasis / unread', kind: '스타일', values: '제목 굵기 / 안 읽음 점(06 §3-7). 합치지 않는다 — 게시판은 공지 행을 굵게 두면서 점은 안 찍어(이미 배지로 구분) 두 값이 갈린다. 점은 제목 앞 인라인이고 leading이 있으면 그 모서리로 옮겨간다(같은 요소·같은 낭독 문구). 좌측 슬롯에 두면 예약=빈 칸 / 미예약=제목 시작선 톱니 중 하나를 반드시 고르게 된다. 선택 모드에선 안 찍는다(체크 옆 점은 선택 상태로 오독)' },
      { name: 'selectable / selected / onSelectedChange', kind: '기능', values: '일괄 처리(선택 모드). 켜지면 좌측에 체크가 서고 chevron이 사라진다(행이 진입을 안 하므로 — 같은 표적이 두 가지로 동작하면 손가락이 어느 쪽인지 모른다). 체크는 leading 자리를 대신한다. 모드의 주인은 화면이고 행은 자기가 선택됐는지만 안다. 진입 경로(길게 누르기·상단 액션)도 화면 책임 — 단 제스처가 유일 경로여선 안 된다(06 §1-4)' },
    ],
    composition: {
  ...
  { name: 'MobileFilterBar', layer: '유기체', role: '축이 둘 이상인 필터.',
    props: [
      ...
      { name: 'hiddenKeys', kind: '기능', values: '`ReadonlySet<string>` — 숨긴 키다(고른 키가 아니다). 극성이 중요하다: 데이터에 새 값이 생기면 여기 없으므로 자동으로 보인다. selected였다면 새 값이 기본 숨김이 되어 소비처가 모른 채 누락된다' },
  ...
  { name: 'MobileRecordList', layer: '유기체', role: '표와 같은 데이터를 폰에서 행으로 그린다.',
    props: [
      { name: 'columns / rows', kind: '기능', values: 'DataTableColumn[] · DataTableRow[] — 표와 같은 배열을 본다. listSlot이 카드 표현을 파생시킨다. kicker는 사다리 3단 자리(드물게 뜨는 것 — 지연·필독)이고, 값이 *모든 행에* 있는 축(공정·담당)은 `inline`이나 섹션으로 내린다(06 §3-6 — 배지밭이 되면 배지가 신호이길 그만둔다). 표현은 열의 `type`이 정한다(여기서 배지를 강제하면 표와 목록이 다른 말을 한다)' },
  ...
  { name: 'MobileList', layer: '유기체', role: '모바일 목록 화면의 껍데기.',
    props: [
      ...
      { name: 'sections / groupBy', kind: '기능', values: '계층은 축이다(평면 / 섹션 / 섹션+그룹). 별도 부품으로 안 가른다 — 가르면 status·필터·더보기 계약이 두 벌이 되고 그게 이 부품이 해결한 문제의 재현이다. 섹션은 0건이어도 헤더가 남는다(구조가 안 흔들리게)' },
      { name: 'renderGroupHeader / renderGroupAction', kind: '콘텐츠', values: '그룹 머리·액션 슬롯. 액션은 그룹 크기 ≥ 2에서만 렌더한다(한 건짜리에 일괄은 뜻이 없다 — 노출 판정은 부품 소관)' },
      { name: 'filters / filter / onFilterChange', kind: '기능', values: '축이 하나일 때의 자리(칩 줄). 둘 이상이면 MobileFilterBar — 축 하나에 드롭다운은 한 번 더 누르게 하고, 축이 여럿인데 칩을 깔면 줄이 늘어난다' },
      ...
      { name: 'onLoadMore / loadMoreLabel / totalCount', kind: '기능', values: '더보기 노출은 데이터가 결정한다 — totalCount를 주면 items.length < totalCount일 때만' },
    ],
    composition: {

...
export const CATALOG: CatalogEntry[] = [
  ...
  { name: 'MobileField', layer: '분자', role: '모바일 폼의 한 칸 — **밑줄 필드**([작은 라벨] / [값·입력] / [아래 헤어라인]). 데스크탑 FormField(상자)를 모바일에서 *대체*한다. 상자만 벗기면 어포던스가 통째로 사라져 값이 그냥 글자로 떠 있게 되는데(실화면), iOS grouped list가 성립하는 건 *행*이 경계를 갖기 때문이다 — 그 경계를 밑줄이 진다(Material filled 필드와 같은 기제, 국내 앱 다수 형태). 라벨은 **위**에 둔다: Baymard가 모바일 inline 라벨을 금하고(좁은 폭에서 입력칸이 쪼그라들고 오류가 는다) 상단 라벨이 완료 시간을 최대 50% 줄인다는 아이트래킹(Wroblewski)이 근거. 포커스는 밑줄 색으로 말한다(아웃라인 상자를 쓰면 벗긴 상자가 포커스 때만 되살아난다).',
    props: [
      { name: 'label', kind: '콘텐츠', values: 'string' },

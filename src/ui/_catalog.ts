...
export const CATALOG: CatalogEntry[] = [
  ...
  { name: 'OptionSetPicker', layer: '유기체', role: '저작 면이 정의한 OptionSet을 읽어 *고르는* 선택 면 위젯 — OptionSetEditor의 짝. **행 규율(실검토 확정)**: 상태는 기하를 못 바꾼다 — 수량 슬롯 상시 예약(빈 행 hover "담기" 알약→같은 자리 스테퍼 morph), 행 소계 없음(품목 소계=풋터), 수량 행 숫자=단가(무부호), 택 델타만 "+ ₩"·포함 사양은 \'포함\', 담긴 행 틴트 없음(✓마크+weight). **표현 어휘(자동+override)**: single→cards(≤6 — 제목+델타, 선택=링 색 승격만)/grid(≤10)/filtered(>10·값묶음)/list/chips/segmented/select · multi→grid/filtered/list/chips/cards · quantity→stepper/filtered/grid(\'collect\'는 지원 중단 별칭=filtered) · number→input(범위 넓음 — blur 스냅)/stepper. **2열 규칙**: 3개 이상=반폭 2열(거터+hairline)·2개 이하=반열 한 열·값묶음 밴드 그룹=1열(밴드×2열 상호배타). **접힘**: defaultCollapsed 기본 \'sequential\'(첫 그룹만 — 신규 작성)·재편집은 \'satisfied\'·"다음" 진행 접힘·선택은 접힘 유발 안 함·collapseOnPick opt-in·openGroups controlled. **시점 보정**: 펼침·다음=최하단 맞춤(패널 내부·하이라이트 없음)/잠금 담기·ref=상단 점프. 풋터 소계=대형 타이포, 미충족 경고=잠금 담기 탭 시 플로팅(2.6s·기하 불변). 그룹 헤더 sticky(불투명)·필수=별표·그룹1+섹션無=헤더 생략. 검색: 소비처 제어, 미충족 필수는 못 지움. 금액 계산 0(§6).',
    props: [
      ...
      { name: 'display?', kind: '스타일', values: "Record<groupId, 'list'|'cards'|'chips'|'grid'|'segmented'|'select'|'filtered'|'stepper'|'input'> — override(생략=자동, selection과 안 맞으면 무시)" },

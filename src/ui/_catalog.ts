...
export const CATALOG: CatalogEntry[] = [
  ...
  { name: 'PaperDoc', layer: '유기체', role: '서식(PaperSpec) + 값 → A4 **여러 장**. 도메인 0줄. 배치(쪽 나눔·반복 펼침·집계·묶음 걸침)는 순수 엔진(paperLayout)이 하고 여기는 그리기만 한다. 24열 격자는 *정렬 골격*이고 **선은 셀의 속성**이다 — 선 소유권(각 칸이 자기 위·왼쪽만 그린다)으로 맞닿은 자리가 2px 되는 걸 막는다. 괘선은 종이 절대색(검정) — 역할 변수를 태우면 음영 칸 위에서 선이 사라진다. `mode: edit`이면 데이터 자리가 입력이 되되 **문서 기하는 그대로**(칸이 곧 입력의 크기, 입력은 자기 크롬을 벗는다). 서식은 `npx erp-paper-import`로 xlsx에서 만든다. **줄마다 깊이가 다른 표**(내역서)는 트리 배열 + `indent`로 담는다(아래 두 항목). ⚠ 2단 그룹 미해결(걸침·그룹머리가 두 층이면 안쪽이 바깥을 따라가 *틀린 값*이 찍힌다) · 흐름 셀 없음.',
    props: [
      ...
      { name: '트리 배열(깊이)', kind: '값', values: '**줄마다 깊이가 다른 표**(내역서·BOM — 「주방 › 상부장 › 옵션1」). 축은 둘이다: 배열이 깊이 열을 가지면(`PaperArray.level` — 엑셀 「필드」 시트에서 종류를 «깊이»로) 그 배열은 트리가 되고, 칸에 `indent`를 주면(`{{들여:품목.품명}}`) 그 줄의 깊이만큼 **글자가** 밀린다. 격자는 안 움직인다 — 칸을 옮기면 열이 어긋나 테두리가 계단이 되고 병합 폭이 깊이마다 달라져 표가 목록으로 읽힌다. 그래서 보통 **품명 칸 하나에만** 붙인다(수량·단가는 제 열에 서야 세로로 읽힌다). 한 계단은 `--paper-indent`(행 단위의 0.5)로 **부품이 정한다** — px를 열면 같은 회사 문서 둘이 다른 계단을 갖는다. 중첩(children)이 아니라 평탄 배열 + 깊이 열인 이유: 엑셀이 이미 그 모양이고(왼쪽 칸에 레벨), `buildHierarchyFromRows` 선례가 같고, 쪽 나눔이 평평한 흐름 위에서 돌기 때문' },
      { name: '트리의 소계(atLevel)', kind: '기능', values: '트리에선 묶음 기준이 열이 아니라 **구조**다 — 「주방」에 무엇이 딸렸는지 말해 주는 열이 없고, **깊이 1인 줄이 다시 나오는 것이 곧 앞 묶음의 끝**이다. 그래서 `groupFooter`가 `by`(값이 같은 줄끼리) 대신 `atLevel`(깊이 ≤ N에서 자름)을 쓴다. 변환기는 트리 반복에 「그룹꼬리」만 있으면 **깊이 1을 기본으로** 준다(그룹머리 없이 소계가 성립한다). `by`와 달리 **재배열이 없다** — 트리는 적힌 순서가 곧 의미라, 같은 이름의 「옵션」이 딴 공사 밑에 또 있어도 한 덩어리로 뭉치면 안 된다. ⚠ 소계는 그 묶음에 딸린 **모든 줄**을 더한다(중간 줄도 자기 금액을 갖는 내역서 전제 — 부모가 자식의 롤업이면 두 번 더해진다). **깊이별 소계는 아직 안 연다**: 반복 하나에 그룹꼬리 하나가 `findClusters`의 전제다' },

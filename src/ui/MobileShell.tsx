...
type Props = {
  ...
  // ── 탭 — **상한 5.** 넘으면 늘리거나 스크롤하지 말고 허브를 도입한다(06 §2-4) ──
  tabs: MobileTab[];
  /**
   * **어느 탭이 활성인가**를 정한다 — *현재 화면의 경로*가 아니다.
   * 판정은 `tab.path === activePath` 정확 일치다(부품이 경로 규칙을 추측하지 않는다).
   *
   * 그래서 **허브(「전체」 탭)를 거쳐 들어간 화면에서는 허브 탭의 path를 넘긴다.**
   * 안 그러면 어느 탭도 안 맞아 탭바에 활성이 하나도 없는 상태가 된다.
   * 소비처는 "지금 어느 화면인가"와 "그 화면을 어느 탭이 데려왔나"를 둘 다 알고 있으므로,
   * 접두 일치 같은 규칙을 부품에 넣는 것보다 이쪽이 정직하다(경로 체계는 도메인이다).
   */
  activePath: string;

...
// 아이콘 광학정렬 보정 — **인라인으로 글자 옆에 놓일 때** 아이콘 중심을 글자 중심에 맞춘다.
//  ⚠ 값이 «1/8 룰(-0.125em)»이었는데 그건 **아이콘 높이가 1em일 때**의 고전값이다.
//   우리 인라인 아이콘은 `sm`=16px 고정이고 본문은 14px이라 **1.14em**이라 비가 안 맞았다.
//   실측(2026-08-21, 8배율 픽셀 스캔 · StatusLabel 문장 안): 아이콘 잉크 중심이 한글 글리프 중심보다
//   **1.25px 위**. 그만큼(1.25/14 = 0.089em) 더 내린다.
//  ⚠ **이 값은 「아이콘 sm(16) + 본문(14)」 짝에 맞춘 것이다.** 한 상수로 모든 짝을 못 맞춘다 —
//   보정량이 «아이콘 높이 ÷ 글자 크기»에 달렸기 때문이다. 다른 짝이 눈에 걸리면 그건 토큰이 아니라
//   그 자리의 문제다(대개 flex로 묶여 있어 이 값이 아예 안 쓰인다 — 그래서 결함이 오래 안 보였다).
const iconBaselineShift = '-0.214em';

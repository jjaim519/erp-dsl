...
//   · A층 — 데이터·콜백 유무
//     트위스티=row.children · 드릴=`onRowClick`(모달 비소유, 신호만) · 펼침=`expandedIds`+`onExpandChange`(controlled) · 기준일
//     캡션=`asOf` · 총액=`total`(없으면 열 합계에서 파생)
//   · B층 — actions
//     툴바 우측. 위젯이 둘 이상인 페이지에서만(그 외엔 PageHeader). 툴바 존은 액션이 없어도 그려진다 — 좌측의 기준 캡션(기준일·만기/발행일)이 없으면 숫자를 못 읽기 때문.
//     `basis`가 바뀌면 표 전체가 다른 수가 된다

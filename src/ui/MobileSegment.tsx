...
//
//  **균등/스크롤을 prop으로도, 개수로도 정하지 않는다 — 여백의 하한이 정한다.**
//   전에는 "3개 이하 균등 / 4개 이상 스크롤"로 개수 분기를 했는데 그게 임의 판단이었다.
//   그렇다고 개수를 지우고 그냥 늘리게 두니, 5탭이 375px에 아슬아슬하게 들어가면서 항목끼리 붙어
//   **"갈래"로 안 읽히는** 상태가 됐다(실화면 지적). 답은 개수가 아니라 **항목마다 최소 여백을 세우는 것**이다.
//   좌우 md를 하한으로 두면 4탭은 남는 폭을 나눠 갖고, 5탭+카운트는 넘쳐 스크롤로 간다 — 개수를 안 세도 같은 판정이 나온다.
//   M3 명문이 그 방향이다: "탭이 안 맞으면 **텍스트를 줄이지 말고 스크롤 탭을 써라**", "라벨을 자르지 마라"
//   (고정 탭은 4개 이하 권장, 5개는 스크롤).

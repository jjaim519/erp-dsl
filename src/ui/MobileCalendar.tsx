...
export function MobileCalendar({
  ...
  // 범례 겸 필터 — 축이 둘이다: 이벤트 분류(anchor) / 태그(annotations). 한 줄에 두되 divider로 가른다.
  //  색만 있고 이름이 없으면 navy가 시공인지, 회색 셀이 무엇인지 알 방법이 없다(데스크탑엔 범례가 있다).
  //  칩은 자기가 거는 *바와 같은 톤*으로 칠한다(Chip variant='legend') — 대응을 배울 필요가 없다.
  ...
  return (
    <div className={fill ? 'mcal fill' : 'mcal'}>
      ...
      <div className="mcal-kinds">
        ...
        {annotations.map((an) => (
          <Chip key={an.id} variant="legend" color={an.tone} selected={!hidden.has('t:' + an.id)} onChange={() => toggle('t:' + an.id)}>
            {an.label}
          </Chip>
        ))}
      </div>

'use client';
// 반응형 프리뷰 갤러리 — AppShell + 페이지를 2티어(데스크탑/태블릿) iframe으로 본다.
//  폰은 이 셸의 범위가 아니다(v0.52.0에서 폰 티어 제거) — 모바일은 /shell/mobile 에서 MobileShell로 본다.
...
//  · 브레이크포인트: 태블릿 768–1279 · 데스크탑 ≥1280(하한 768 미만은 지원 밖). 1280 경계 = iPad 가로(≤Pro11 1194)를
...
const TIER: Record<string, { w: number; h: number }> = { desktop: { w: 1440, h: 900 }, tablet: { w: 1024, h: 768 } };
...
function tierOf(w: number): { label: string; color: 'neutral' | 'info' | 'success' } {
  if (w < 768) return { label: '지원 밖 (모바일은 MobileShell)', color: 'neutral' };
...
export default function PreviewGallery() {
  ...
  const applyTier = (t: string) => {
    setTier(t);
    const base = t === 'tablet' ? TABLET_ORIENT[orient] : TIER[t];
    ...
    <Stack gap="lg">
      <Stack gap="xs">
        ...
        <Text variant="body" color="secondary">
          AppShell + 페이지를 iframe(자체 뷰포트)으로 띄워 2티어를 진짜 폭에서 본다. 브레이크포인트: 태블릿 768–1279 · 데스크탑 ≥1280. 768 미만은 지원 밖 — 모바일은 <code>/shell/mobile</code>.
        </Text>
      </Stack>
          ...
          <Group gap="lg" align="end" wrap>
            <div>
              ...
              <SegmentedControl size="sm" value={tier} onChange={applyTier}
                options={[{ label: '데스크탑', value: 'desktop' }, { label: '태블릿', value: 'tablet' }]} />
            </div>
            {tier === 'tablet' && (
              <div>

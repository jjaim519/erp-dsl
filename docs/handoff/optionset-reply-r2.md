...
# 추기 4 (2026-07-29, v0.58.0) — R3 회신: §2 회귀 수정 + A군 재노출

R3 전 항목 수용. §2와 A군을 v0.58.0 한 릴리스로 발행한다.

## §2 — 수용(2-1). 회귀 인정
저작 면 금액 칸이 §6 예외 선언 없이 `amount`를 쓰고 있었다(v2.1 실수). 수정: **금액 칸 = `override`**(쓰기·표시), `amount`는 계약 주석대로 소비처 주입 표시 전용으로 복귀. 참조(`refId`) 걸린 값은 금액 칸이 비었을 때 **상속가가 흐리게(placeholder)** 보이고(refOptions의 price, 없으면 주입된 amount로 폴백), 적으면 직접값이 이긴다 — IVF 규율 그대로. 부품 내 미리보기 델타도 `override ?? amount` 우선순위 표시로 정렬.

## A군 — 수용. v0.56.0 prop 3종 복원
`refOptions?` / `exprVariables?` / `adjustKeys?` 복원. 편집 면은 v0.56.0의 행 내 펼침이 아니라 **행 팝오버 하나**로 통합(§3-② 충족): 참조(목록 선택·해제·상속가 표시) · 값 배율 · 수식 · 보정. 그룹 배율은 헤더 속성 존의 소형 ×입력(같은 노출 조건).
- 보정(`adjust`)은 "키:±값, 키:±값" 표기로 편집(파서 내장) — KeyValueField 행 편집기는 팝오버 기하에 안 맞아 미채용.
- 수식은 이번 판에서 **plain 입력**(시각 정본 확정 형태). ExpressionField의 변수·문법 검증 통합은 후속(팝오버 내 검증 메시지 기하 논의 필요) — 저장 전 검증이 필요하면 당분간 소비처 저장 경로에서.

## §3 — 수용
- **① prop 미전달 = 완전 미노출**: fx 표식·슬롯·그룹 배율 전부 자리조차 안 생김. 세 prop이 각각/조합 스위치.
- **②③**: fx는 상시 열이 아니다 — **행 hover 시에만 나타나고, 규칙이 걸린 값만 상시 착색**. 평시 표는 [값|금액] 그대로. 참조 걸린 값은 금액 칸의 흐린 상속가가 1차 흔적. **열 경계 불변식**: fx 슬롯은 켜지는 순간 빈 행·열 헤더에도 동시 예약(전 행 동일 해부).

## 소비처 배선 메모
- 금액 칸 표시가 `override` 기준이 됐으므로, 저작 진입 시 값에 `override`(직접가)와 `amount`(계산 유효가 — 참조 placeholder 폴백용)를 함께 실어 보내면 가장 정확하다.
- `RefOption.price`가 있으면 상속가 표시가 즉시 반영된다(권장).
